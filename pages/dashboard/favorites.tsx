...
export default function FavoritesPage() {
  ...
  const totalPages = Math.ceil(sortedFavorites.length / pageSize)
  ...
  return (
    <DashboardLayout>
      <Head>
        <title>我的收藏 - 兔图</title>
        <meta name="description" content="管理您在兔图内容管理平台上收藏的内容" />
      </Head>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-gray-600">
            管理您收藏的内容，共 {filteredFavorites.length} 项
          </p>
        </div>
        ...
        {isFavoritesLoading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mt-8">
            {paginatedFavorites.map((content) => (
              <ContentCard
                key={content.id}
                content={content}
                onRemoveFavorite={() => handleRemoveFavorite(content.uuid)}
