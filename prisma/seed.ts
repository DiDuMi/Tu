...
async function main() {
  ...
  // 创建用户组
  const _guestGroup = await prisma.userGroup.create({
    data: {
      name: '游客',
  ...
  const _monthlyMemberGroup = await prisma.userGroup.create({
    data: {
      name: '月度会员',
  ...
  const _yearlyMemberGroup = await prisma.userGroup.create({
    data: {
      name: '年度会员',
  ...
  const _lifetimeMemberGroup = await prisma.userGroup.create({
    data: {
      name: '终身会员',
  ...
  const user = await prisma.user.create({
    data: {
      ...
      role: 'MEMBER',
      status: 'ACTIVE',
      userGroupId: registeredGroup.id, // 使用注册用户组
    },
  })
  ...
  await prisma.pageTag.createMany({
    data: [
      ...
      { pageId: page3.id, tagId: tags[2].id }, // 教程
  ...
  await prisma.like.createMany({
    data: [
      ...
      { pageId: page1.id, userId: operator.id },
      ...
      { pageId: page6.id, userId: operator.id },
      { pageId: page7.id, userId: admin.id },
      { pageId: page7.id, userId: operator.id },
    ],
  })
